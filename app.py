...
# Set the page config as the first Streamlit command for a professional look
st.set_page_config(page_title="Airline Price Advisor", layout="wide", initial_sidebar_state="expanded")


# --- 1. Load Artifacts and Data ---
# Use cache_data for efficiency so this block only runs once.
@st.cache_data
def load_data_and_artifacts():
    """
    Loads the dataset, pre-trained model, and preprocessor from disk.
    """
    df = pd.read_csv('data/Clean_Dataset_EDA_Processed.csv.csv')
...
try:
    ...
    st.error("Model, preprocessor, or data files not found. Please run the modeling notebook first.")
    st.stop()
