...
with col1:
    ...
    if not route_df.empty:
        ...
        airline = st.selectbox("Airline", [], disabled=True)
...
with col2:
    ...
    if not route_df.empty:
        ...
        time_choice = st.radio("Select by:", ("Departure Time", "Arrival Time"), horizontal=True, key="time_choice")

        if time_choice == "Departure Time":
            departure_time = st.selectbox("Departure Time", sorted(flight_options_df['departure_time'].unique()), key="departure")
            possible_arrivals = sorted(flight_options_df[flight_options_df['departure_time'] == departure_time]['arrival_time'].unique())
            arrival_time = st.selectbox("Arrival Time", possible_arrivals, disabled=True, key="arrival")
        else: # Arrival Time was chosen
            arrival_time = st.selectbox("Arrival Time", sorted(flight_options_df['arrival_time'].unique()), key="arrival")
            possible_departures = sorted(flight_options_df[flight_options_df['arrival_time'] == arrival_time]['departure_time'].unique())
            departure_time = st.selectbox("Departure Time", possible_departures, disabled=True, key="departure")
        
        # --- Dynamic Duration Slider ---
        min_duration = flight_options_df['duration'].min()
        max_duration = flight_options_df['duration'].max()

        # THE FIX: Ensure all range arguments are floats
        duration = st.slider("Flight Duration (in hours)", 
                             min_value=float(math.floor(min_duration)), 
                             max_value=float(math.ceil(max_duration)),
                             value=round(flight_options_df['duration'].mean(), 1),
                             step=0.5)
    else:
        st.radio("Select by:", ("Departure Time", "Arrival Time"), horizontal=True, disabled=True)
        st.slider("Flight Duration (in hours)", 0.0, 50.0, disabled=True)
...
with col3:
    ...
    if not route_df.empty:
        ...
        st.info("Select a valid route to begin.")
